...
import Component from '../src/pages/oj/views/user/LearningFeedback.vue'
import * as data from '../src/pages/oj/views/user/learningFeedbackData.js'
import UserHome from '../src/pages/oj/views/user/UserHome.vue'
const require = createRequire(import.meta.url)
const injected = vi.hoisted(() => ({ api: {}, axios: {} }))
vi.mock('../src/pages/oj/views/user/learningFeedbackApi.js', () => ({ default: new Proxy({}, { get: (_, key) => (...args) => injected.api[key](...args) }) }))
...
const actualApi = await vi.importActual('../src/pages/oj/views/user/learningFeedbackApi.js')
...
function load (filename, imports = {}, mock = true) {
  ...
  if (imports['./learningFeedbackApi']) injected.api = imports['./learningFeedbackApi']
  if (filename === 'LearningFeedback.vue') return { default: Component }
  if (filename === 'learningFeedbackData.js') return data
  if (filename === 'UserHome.vue') return { default: UserHome }
  if (filename === 'learningFeedbackApi.js') return actualApi
...
async function main () {
  ...
  const state = reactiveState({ data: { route: { query: {} }, user: { id: 1, username: 'mock_student' } } })
  ...
  require('../build/mock-learning-feedback')({ get: (url, handler) => { handlers['get ' + url] = handler }, post: (url, handler) => { handlers['post ' + url] = handler } }, context)
...
test('feedback adaptation, lifecycle, API and mock boundaries', main, 10000)
