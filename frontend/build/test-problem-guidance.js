'use strict'
// Run from frontend: node build/test-problem-guidance.js
...
function load (name, imports, mock = true) {
  let source = fs.readFileSync(path.join(__dirname, '../src/pages/oj/views/problem', name), 'utf8')
  ...
  vm.runInNewContext(code, { module, exports: module.exports, require: name => imports[name], process: { env: { AI_FEATURES_MOCK: mock } }, setInterval, clearInterval })
  return module.exports
}
const data = load('problemGuidanceData.js', {})
...
async function settle () { await Vue.nextTick(); await Promise.resolve(); await Promise.resolve() }
async function componentTests () {
  ...
  const state = new Vue({ data: { query: {}, user: { id: 1 } } })
...
async function mockTests () {
  ...
  const module = { exports: {} }
  vm.runInNewContext(fs.readFileSync(path.join(__dirname, 'mock-problem-guidance.js'), 'utf8'), { module, Date: Clock, setTimeout })
  let loggedIn = true
