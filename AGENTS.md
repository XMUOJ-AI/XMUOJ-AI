...
前端采用 Vue 3、Vue Router 4、Vuex 4 和 Vite，保留 JavaScript 和 Options API；学生端使用 View UI Plus，管理端使用 Element Plus。后端采用 Django 3.2、Django REST Framework，通过 PostgreSQL 保存数据，使用 Redis 和 Dramatiq 支撑缓存及后台任务。执行代码的判题服务器和判题沙箱属于外部配套组件，不包含在此仓库中。

## 项目简要导览

- `README.md`：整个项目的说明、环境前提和开发入口。
- `frontend/`：前端项目；`src/pages/oj/` 为用户页面，`src/pages/admin/` 为管理页面，`src/store/` 为状态管理，`vite.config.mjs` 和 `build/` 为开发、构建与 Mock 配置，`tests/` 为 Vitest 回归测试。
...
命令必须在表格指定的目录执行。前端统一使用 Node.js 24、npm 和 `package-lock.json`；使用 `npm ci` 安装，不切换包管理器或无关更新依赖。不再使用 DLL 构建和 OpenSSL 兼容参数。

| 工作目录 | 命令 | 用途 |
| --- | --- | --- |
| `frontend/` | `npm ci` | 按锁文件安装前端依赖 |
| `frontend/` | `TARGET=http://127.0.0.1:8000 npm run dev` | 启动前端并代理后端接口 |
| `frontend/` | `npm run dev:mock` | 启动本地 Mock 预览，无需后端 |
| `frontend/` | `npm run build` | 构建生产静态资源 |
| `frontend/` | `npm run lint` | 检查前端代码 |
| `frontend/` | `npm test` | 运行 Vue 3 组件和业务回归测试 |
