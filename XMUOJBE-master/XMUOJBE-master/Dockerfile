...
RUN --mount=type=cache,target=/etc/apk/cache,id=apk-cahce-$TARGETARCH$TARGETVARIANT-final \
    ...
    sh -c "set -ex && apk add gcc libc-dev python3-dev libpq libpq-dev libjpeg-turbo libjpeg-turbo-dev zlib zlib-dev freetype freetype-dev supervisor openssl nginx curl unzip && pip install -r /app/deploy/requirements.txt && pip install googletrans && apk del gcc libc-dev python3-dev libpq-dev libjpeg-turbo-dev zlib-dev freetype-dev"

COPY ./ /app/
COPY ../OnlineJudgeFE/dist/ /app/dist/
